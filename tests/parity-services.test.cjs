...
test('deleted messages never sign or reveal attachment links; readable attachments use private signed URLs',async()=>{
 const signed=[],q={select(){return q},eq(){return q},is(){return q},order(){return q},then(ok){return Promise.resolve({data:[{id:'deleted',sender_id:'other',body:'secret',deleted_for_everyone_at:'2026-09-01',message_attachments:[{id:'a',storage_path:'secret',upload_state:'complete',deleted_at:null}]},{id:'visible',sender_id:'other',body:'hello',deleted_for_everyone_at:null,message_attachments:[{id:'b',storage_path:'allowed',upload_state:'complete',deleted_at:null,original_name:'photo.jpg',kind:'image',size_bytes:1234},{id:'c',storage_path:'incomplete',upload_state:'uploading',deleted_at:null}]}],error:null}).then(ok)}};
 ...
 const api=load('src/services/web-features.ts',{'@/lib/supabase':{supabase}}),rows=await api.messages('conversation');assert.equal(rows.length,1);assert.equal(rows[0].id,'visible');assert.equal(rows[0].body,'hello');assert.deepEqual(signed,[['chat-attachments','allowed',3600]]);assert.equal(rows[0].attachments.length,1);assert.equal(rows[0].attachments[0].sizeBytes,1234);
});
